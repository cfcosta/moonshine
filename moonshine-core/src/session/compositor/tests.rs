...
impl Harness {
	pub(super) fn add_client(&mut self) -> Peer {
		...
		let server_client = self.display.handle().insert_client(server_socket, Arc::new(ServerClientState {
		...
		let mut old_peer = Peer {
			server_client, connection, queue, qh, client: TestClient::default(),
			sync_requested: 0, buffers: vec![],
		};
	...
	pub(super) fn new() -> Self {
		...
		let server_client = dh.insert_client(server_socket, Arc::new(ServerClientState {
			compositor_state: CompositorClientState::default(),
		...
		let mut harness = Self {
			state, display, _event_loop: event_loop, server_client, connection, queue, qh,
			client: TestClient::default(), sync_requested: 0, buffers: vec![],
		};
		harness.roundtrip();
		harness.roundtrip();
	...
	pub(super) fn roundtrip(&mut self) {
		...
		while self.client.sync_done < target {
			...
			self.display.dispatch_clients(&mut self.state).expect("dispatch test server");
			self.state.refresh_popups();
			self.display.flush_clients().expect("flush test server");
			self.queue.dispatch_pending(&mut self.client).expect("dispatch pending client events");
			...
			self.queue.dispatch_pending(&mut self.client).expect("dispatch test client events");
		}
	}

	pub(super) fn toplevel(&mut self) -> ClientSurface {
		let surface = self.client.compositor.as_ref().unwrap().create_surface(&self.qh, ());
		let xdg_surface = self.client.shell.as_ref().unwrap().get_xdg_surface(&surface, &self.qh, ());
		let toplevel = xdg_surface.get_toplevel(&self.qh, ());
		surface.commit();
		self.roundtrip();
		ClientSurface { surface, xdg_surface, toplevel }
	}
	...
	pub(super) fn uncommitted_popup(&mut self, parent: &xdg_surface::XdgSurface, geometry: (i32, i32, i32, i32)) -> ClientPopup {
		let surface = self.client.compositor.as_ref().unwrap().create_surface(&self.qh, ());
		let xdg_surface = self.client.shell.as_ref().unwrap().get_xdg_surface(&surface, &self.qh, ());
	...
	pub(super) fn map(&mut self, surface: &wl_surface::WlSurface, width: i32, height: i32) {
		...
		let pool = self.client.shm.as_ref().unwrap().create_pool(file.as_fd(), length, &self.qh, ());
	...
	pub(super) fn server_surface(&self, surface: &wl_surface::WlSurface) -> ServerSurface {
		self.server_client.object_from_protocol_id(&self.display.handle(), surface.id().protocol_id())
	...
	pub(super) fn move_pointer(&mut self, x: i16, y: i16) {
		self.input(CompositorInputEvent::MouseMoveAbsolute { x, y, screen_width: 800, screen_height: 600 });
	}

	pub(super) fn press_pointer(&mut self) -> u32 {
		self.input(CompositorInputEvent::MouseButtonDown { button: 0x110 });
		self.client.events.iter().rev().find_map(|event| match event {
	...
	pub(super) fn request_presentation(&self, surface: &wl_surface::WlSurface) -> u32 {
		let feedback = self.client.presentation.as_ref().expect("presentation global").feedback(surface, &self.qh, ());
...
impl Dispatch<wl_callback::WlCallback, u32> for TestClient {
	fn event(state: &mut Self, callback: &wl_callback::WlCallback, _: wl_callback::Event, cookie: &u32, _: &Connection, _: &QueueHandle<Self>) {
		if *cookie == 0 {
...
impl Dispatch<wp_presentation_feedback::WpPresentationFeedback, ()> for TestClient {
	fn event(state: &mut Self, feedback: &wp_presentation_feedback::WpPresentationFeedback, event: wp_presentation_feedback::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
		let id = feedback.id().protocol_id();
		match event {
...
impl Dispatch<xdg_wm_base::XdgWmBase, ()> for TestClient {
	fn event(_: &mut Self, shell: &xdg_wm_base::XdgWmBase, event: xdg_wm_base::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
		if let xdg_wm_base::Event::Ping { serial } = event { shell.pong(serial); }
	}
}
...
impl Dispatch<xdg_surface::XdgSurface, ()> for TestClient {
	fn event(state: &mut Self, surface: &xdg_surface::XdgSurface, event: xdg_surface::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
		if let xdg_surface::Event::Configure { serial } = event {
			state.events.push(ClientEvent::Configure(surface.id().protocol_id(), serial));
...
impl Dispatch<xdg_popup::XdgPopup, ()> for TestClient {
	fn event(state: &mut Self, popup: &xdg_popup::XdgPopup, event: xdg_popup::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
		let id = popup.id().protocol_id();
		match event {
			xdg_popup::Event::Configure { x, y, width, height } => state.events.push(ClientEvent::PopupConfigure(id, (x, y, width, height))),
...
impl Dispatch<wl_pointer::WlPointer, ()> for TestClient {
	fn event(state: &mut Self, _: &wl_pointer::WlPointer, event: wl_pointer::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
		match event {
			...
			wl_pointer::Event::Motion { surface_x, surface_y, .. } => state.events.push(ClientEvent::PointerMotion(state.pointer_focus, surface_x, surface_y)),
			wl_pointer::Event::Button { serial, button, state: pressed, .. } => state.events.push(ClientEvent::PointerButton(state.pointer_focus, serial, button, pressed.into())),
			_ => {},
		}
	}
}
...
impl Dispatch<wl_keyboard::WlKeyboard, ()> for TestClient {
	fn event(state: &mut Self, _: &wl_keyboard::WlKeyboard, event: wl_keyboard::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
		match event {
			...
			wl_keyboard::Event::Key { serial, key, state: pressed, .. } => state.events.push(ClientEvent::Key(state.keyboard_focus, serial, key, pressed.into())),
			_ => {},
		}
	}
}
...
impl Dispatch<wl_touch::WlTouch, ()> for TestClient {
	fn event(state: &mut Self, _: &wl_touch::WlTouch, event: wl_touch::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
		match event {
			wl_touch::Event::Down { serial, surface, id, x, y, .. } => state.events.push(ClientEvent::TouchDown(surface.id().protocol_id(), serial, id, x, y)),
...
fn popup_initial_configure_is_sent_after_initial_surface_commit() {
	...
	assert!(!harness.client.events.iter().any(|event| matches!(event, ClientEvent::PopupConfigure(id, _) if *id == popup_id)), "popup must wait for initial wl_surface.commit");
	popup.surface.commit();
	harness.roundtrip();
	assert!(harness.client.events.contains(&ClientEvent::PopupConfigure(popup_id, (120, 80, 160, 100))), "first commit must configure the requested popup geometry; events: {:?}", harness.client.events);
	assert!(harness.client.events.iter().any(|event| matches!(event, ClientEvent::Configure(id, _) if *id == popup.xdg_surface.id().protocol_id())), "xdg_surface.configure must follow xdg_popup.configure");
	let parent_surface = harness.server_surface(&parent.surface);
	assert_eq!(PopupManager::popups_for_surface(&parent_surface).count(), 1, "popup must be registered for rendering and hit testing");
}
