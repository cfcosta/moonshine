...
fn popup_implicit_touch_recipient_cannot_authorize_the_client_under_another_finger() {
	...
	h.input(CompositorInputEvent::TouchDown { slot: 0, x: 0.125, y: 0.25 });
	h.input(CompositorInputEvent::TouchDown { slot: 1, x: 0.75, y: 0.25 });
	assert!(!h.client.events.iter().any(|event| matches!(event, ClientEvent::TouchDown(..))),
		"the hit-tested second client must not receive either implicitly grabbed contact");
	h.swap_client(&mut first_client);
	h.roundtrip();
	let serial = h.client.events.iter().find_map(|event| match event {
		ClientEvent::TouchDown(surface, serial, 1, _, _) if *surface == first_root.surface.id().protocol_id() => Some(*serial),
		_ => None,
	...
	menu.surface.commit();
	h.roundtrip();
	assert!(h.client.events.contains(&ClientEvent::PopupDone(menu.popup.id().protocol_id())),
